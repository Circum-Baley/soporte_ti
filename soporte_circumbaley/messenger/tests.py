...
class ThreadTestCase(TestCase):
    ...
    def test_add_users_to_thread(self):
        ...
        self.assertEqual(len(self.thread.users.all()), 3)
    ...
    def test_add_messages_to_threads(self):
        self.thread.users.add(self.user1, self.user2)
        message1 = Message.objects.create(user=self.user1, content="Mensaje Uno 1")
        message2 = Message.objects.create(user=self.user1, content="Mensaje Dos 2")
    ...
    def test_add_message_from_user_not_in_thread(self):
        self.thread.users.add(self.user1, self.user2)
        message1 = Message.objects.create(user=self.user1, content="Mensaje uno 1")
        message2 = Message.objects.create(user=self.user2, content="Mensaje dos 2")
        message3 = Message.objects.create(user=self.user3, content="Mensaje tres 3")
